...
For non-Gaussian responses (binary, count, proportional data) or when
you need random effects beyond phylogeny, use a PGLMM. The MCMCglmm
package fits Bayesian phylogenetic mixed models:

```{r pglmm, eval = FALSE}
library(MCMCglmm)

# Prepare: species as a factor matching tree tip labels
aligned$data$phylo <- gsub(" ", "_", aligned$data$Species1)

# Inverse phylogenetic covariance matrix
inv_phylo <- inverseA(aligned$tree, nodes = "TIPS", scale = TRUE)

# PGLMM: binary response (e.g., migration status)
prior <- list(R = list(V = 1, fix = 1),
              G = list(G1 = list(V = 1, nu = 0.002)))

model <- MCMCglmm(
  Migration ~ log(Mass) + Trophic.Level,
  random = ~phylo,
  family = "categorical",
...
details, and Nakagawa & Schielzeth (2013, *J. Evol. Biol.*
23:494--508) and Villemereuil et al. (2025, *J. Evol. Biol.*
38:1699--1723) for guidance on phylogenetic mixed models and their
interpretation.
