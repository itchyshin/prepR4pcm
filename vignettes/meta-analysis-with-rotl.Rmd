...
![Left: the 14-tip Open Tree of Life topology before grafting. Right: the same tree after `reconcile_augment()` grafts *Anolis carolinensis* (15 tips). The new species is added as a bifurcating sister to its congener *Anolis sagrei*, so the tree stays strictly bifurcating and needs no follow-up `multi2di()`.](../man/figures/meta-analysis-graft-compare.png){width=100%}
