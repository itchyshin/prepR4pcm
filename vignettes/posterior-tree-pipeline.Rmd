...
share `my_topology` but have *different branch lengths*, where each
variant comes from a different source paper in DateLife's database
(e.g. variant 1 dated against Hedges et al. 2015, variant 2
against Bininda-Emonds et al. 2007, etc.). To get N different
*topologies*, fetch them separately first (e.g.
`pr_get_tree(species, source = "rtrees", taxon = "mammal")` returns
~100 topologies), then pass that `multiPhylo` to `pr_date_tree()`
to get the topology × source cross-product.
