...
The supported authorities (all served via [taxadb](https://docs.ropensci.org/taxadb/)):
...
- **`itis`** — Integrated Taxonomic Information System
  ([itis.gov](https://www.itis.gov/)). North-American emphasis,
  strong on vertebrates and vascular plants.
- **`gbif`** — Global Biodiversity Information Facility taxonomic
  backbone ([gbif.org/dataset](https://www.gbif.org/dataset/d7dddbf4-2cf0-4f39-9b2a-bb099caae36c)).
...
- **`ott`** — Open Tree Taxonomy
  ([opentreeoflife.org/about/taxonomy-version](https://tree.opentreeoflife.org/about/taxonomy-version)).
  The taxonomy used by Open Tree of Life, useful when paired with
  the `rotl` retrieval backend.
- **`itis_test`** — small bundled subset of ITIS used for the
  package's own examples and tests; not a general-purpose authority.

These mirror the providers documented in `?taxadb::td_create`. The
database is downloaded once on first use and cached locally,
ensuring reproducibility (results don't depend on an external API
being available the day you run the analysis).
...
Set `authority = "col"` (or another provider) when names differ
because of genuine taxonomic revisions — species moved to a
