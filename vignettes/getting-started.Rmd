...
The authorities below are **databases served by the
[taxadb](https://docs.ropensci.org/taxadb/) package** (Norman et al.
2020). They are not R packages themselves — `authority = "col"`
just tells `prepR4pcm` to look up synonyms in the taxadb-cached
copy of the Catalogue of Life database, and so on for the others.
The first call for a given authority downloads its database to
your local taxadb cache (~100 MB); subsequent calls are fast and
work offline.
...
These mirror the providers documented in `?taxadb::td_create`.
